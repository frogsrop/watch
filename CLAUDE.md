...
- **kinomix.web.app** (`api.kinobox.tv` агрегатор → Collaps → `api.ortified.ws` venom) — Variant E. `api.kinobox.tv` блокирует datacenter IPs через **TLS-фингерпринтинг** (HTTP/2 PROTOCOL_ERROR даже из реального Chrome на VPS, undici / curl тоже не проходят — но fetch из контекста уже загруженной kinomix.web.app проходит). Воркэраунд: `scripts/crawl-kinomix.mjs <kp_id>` локально через Playwright делает `page.evaluate(() => fetch('/api/players?kinopoisk=N'))` уже после загрузки kinomix.web.app, парсит ответ, сохраняет mapping `kinopoisk_id → ortified_id` в `data/kinomix-cache.json`. На проде `captureFromKinomix` читает кеш, undici-fetch'ит `api.ortified.ws/embed/movie/<id>` (этот доступен), потом тот же `extractVenomSeasons()` что для lordfilm. Контент стримится с `*.interkh.com` (общий CDN с lordfilm) и `*.rtbcdn.cloud` — оба в whitelist.
