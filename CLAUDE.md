...
| Окно QUIC-потока | `http3_stream_buffer_size 256k` + `http3_max_concurrent_streams 32` в vhost | goodput сегментов по h3 — см. ниже, **4.2×** от дефолта |
| `quic_host_key` | `/etc/nginx/quic_host.key` (32 байта, root:root 0600) | `sudo grep quic_host_key /etc/nginx/sites-available/frogsrop.dev` |
| Цепочка сертификата без корня | `/etc/nginx/ssl/frogsrop.dev/fullchain.crt` — 3 сертификата вместо 4 | `echo \| openssl s_client -connect frogsrop.dev:443 -servername frogsrop.dev -showcerts 2>/dev/null \| grep -c BEGIN` → `3` |
| Протокол и тайминги в одном логе | `log_format watch` в conf.d (поле `proto=$http3`), `access_log` во всех трёх наших локациях | `sudo awk '{print $7}' /var/log/nginx/watch.log \| sort \| uniq -c` |
| Alt-Svc из одного места | `/etc/nginx/snippets/watch-alt-svc.conf`, `include` в server{} и в сегментной локации | `sudo nginx -T \| grep -c Alt-Svc` → `2` |
...
Ловушки при отладке QUIC, обе стоили времени:
- Серверный `error_log` **внутри server-блока** забирает QUIC-события себе, а в
  главном логе остаётся только шум epoll — легко решить, что nginx пакеты вообще
  не обрабатывает.
- Правильный инструмент — **`debug_connection <ip>;` в блоке `events{}`**:
  подробный лог включается только для одного клиента, глобальный уровень
  поднимать не нужно. Сборка Debian идёт `--with-debug`, так что доступно сразу.

**«Остаточные отказы h3» оказались артефактом curl, а не дефектом сервера** — тут
дважды было записано неверно, пишу окончательно. Симптом: `curl --http3-only`
отдаёт `http_code 000`, `size_download 0`, `time_starttransfer 0.000000` при
`num_connects 1`, то есть соединение есть, запрос ушёл, обратно якобы ничего.
Доля ~20%, воспроизводится и на `127.0.0.1`.

Разобрано в изолированном nginx на 8443 (`debug_connection 127.0.0.1`), и трейс
показывает обратное:

```
quic path seq:0 status tx:114306 rx:6363 valid:1 st:2 mtu:1200
quic frame rx app:6 CONNECTION_CLOSE_APP err:0     ← закрыл КЛИЕНТ, код 0
quic state: draining
```

nginx передал **114 306 байт** для файла в 102 400 — то есть ответ отдал целиком,
а потом **клиент прислал CONNECTION_CLOSE с кодом «без ошибки»**. `error:1` в
`quic close immediate` — внутренняя пометка «закрываюсь по полученному close», не
ошибка nginx.

Что проверено и отвергнуто как причина: **многоворкерность** — 4 воркера дали 16
отказов из 75, один воркер 15 из 75, разницы нет, значит `quic_bpf` не поможет и
`reuseport` остаток не объясняет; **размер ответа** — 100 КБ / 413 КБ / 3.5 МБ
дали 20% / 12% / 32%, монотонности нет; **переполнение UDP-буферов** —
`UdpInErrors`/`UdpRcvbufErrors`/`UdpSndbufErrors` ровно нули до и после серии;
`quic_gso` (9/10 и с ним, и без); `quic_retry` (16/25 против 15/25); залипшие
после `reload` воркеры (полный `restart` — 18/25); темп запросов (с паузой 1 с —
18/20). Воспроизведение на статическом файле в лаборатории заодно снимает
подозрения с приложения, Node и HMAC-прокси.

**Настоящий браузер этого не видит.** Замер в покое (без свежих `reload`):
**24 запроса по h3 против 1 по h2, все 18 сегментов по h3, ноль отказов**. Более
раннее «1 против 94» было артефактом моих же перезагрузок nginx, которые рвали
h3-соединения посреди сеанса и загоняли Chrome в фолбэк. Так что гоняться за
этими 20% незачем — их видит только curl.

### Почему h3 не был быстрее h2 — и как это починено (2026-08-03)

Причина была одна и в конфиге: **`http3_stream_buffer_size` по умолчанию 64k**.
`ngx_quic_stream_send_chain()` держит `sent − acked ≤ этого значения`, поэтому
один поток не может обогнать `буфер / RTT` независимо от congestion window. hls.js
тянет фрагменты по одному, так что этот пер-поточный потолок и есть пропускная
способность соединения.

Замер до и после на одном и том же пути (RTT 55 мс), сегменты >1 МБ по h3, из
`log_format watch`:

| | goodput (p50) | предсказание `буфер/RTT` |
|---|---|---|
| `http3_stream_buffer_size 64k` | 0.99 МБ/с = **7.9 Мбит/с** (n=124) | 9.5 Мбит/с |
| `http3_stream_buffer_size 256k` | 4.15 МБ/с = **33.2 Мбит/с** (n=51, p10 4.01 / p90 4.25) | 38 Мбит/с |

**4.2×**, и оба замера садятся на ~85% от теоретического потолка — модель
подтверждена. Клиентская телеметрия согласна: `skb*8/sms` дал 8.2–11.5 Мбит/с до
и 36.8–38.8 Мбит/с после. Буфер набирается до 182 с за 46 с воспроизведения
против 45 с к 50-й секунде на дефолте. Выпавших кадров 0, ресинков 0.

Важно для дальних зрителей: 33 Мбит/с сильно выше потолка 20.6 Мбит/с, который
эта же настройка даёт при loaded RTT 102 мс, то есть **окно перестало быть
ограничителем для всех** — узким местом снова стал канал зрителя (~10 Мбит/с),
как и должно быть.

Почему **не** 1024k, на котором сошлись в nginx/nginx#217: это фиксированное
окно, а пейсинга у nginx нет ни в одной версии, поэтому всё выше BDP уходит не в
goodput, а в стоячую очередь — 1024k наводит ~780 мс на канале 10 Мбит/с, это вне
`DRIFT_DEADZONE_S` (0.3 с) из `player.js` и начнёт дёргать `playbackRate` у всей
комнаты через тот же канал, где идёт WebSocket-синхронизация. 256k даёт ~155 мс —
внутри мёртвой зоны. Следующий и последний шаг, если дальний зритель всё ещё
зажат, — 384k.

`http3_max_concurrent_streams 32` **двигается вместе с буфером, никогда
отдельно**: `initial_max_data = (streams + 3) × buffer`, так что поднять буфер в
одиночку — это ×4 к тому, сколько памяти может заставить держать один клиент, на
ручке `/api/extract` без авторизации. `(32+3)×256k = 8.75 MiB` против
`(128+3)×64k = 8.19 MiB` — та же экспозиция, перераспределённая в меньшее число
более толстых потоков.

Что при этом **не** трогали и почему: kernel BBR к QUIC не применяется вообще
(QUIC живёт в userspace, у nginx свой NewReno без возможности замены), пейсинга
нет ни в одной версии, 0-RTT скомпилирован выключенным на OpenSSL-compat-слое
(`SSL_set_max_early_data(ssl, 0)`), а апгрейд nginx отвергнут: CUBIC из 1.27.5
имеет воспроизводимую регрессию под потерями (47 МБ при 1% loss: 21.3 с → 221.7 с),
и риск ложится на машину с почтой.

### Рукопожатие: что сделано и что отвергнуто по счёту

**`quic_host_key` поставлен.** Без него nginx генерирует случайный ключ при каждом
разборе конфига, то есть на **каждый `reload`**, и документация прямо говорит:
токены, выданные старым ключом, не принимаются. При включённом `quic_retry` это
значит, что любой reload обесценивает все выданные Retry/`NEW_TOKEN` и
stateless-reset токены. Vhost общий с тремя сервисами — reload'ится при любом их
изменении. Ключ читает мастер до fork'а, поэтому 0600 root:root корректно и он
общий для воркеров (а НЕ пер-воркерный, как можно было заподозрить по симптомам).

**`quic_retry off` отвергнут по арифметике, а не по вкусу.** Бюджет
анти-амплификации RFC 9000 — 3× полученного, то есть ~3600 байт на клиентский
Initial в 1200. Первый flight сервера — ~5 КБ сертификатов плюс
ServerHello/EncryptedExtensions/CertificateVerify (256 байт подписи
RSA-2048)/Finished. Значит сервер **уже** встаёт посреди flight'а в ожидании ACK
клиента, а это ровно тот round-trip, который снятие Retry должно было сэкономить.
Чистый выигрыш — ноль. Настоящее решение — ECDSA-цепочка (~2 КБ, влезает в
бюджет), это работа с сертификатом, а не с QUIC.

**Из цепочки убран лишний корень.** Сервер отдавал 4 сертификата, 6499 байт DER:

```
1  frogsrop.dev              ← Sectigo CA DV R36     1688 Б  лист
2  Sectigo CA DV R36         ← Sectigo Root R46      1616 Б  промежуточный
3  Sectigo Root R46          ← USERTrust RSA CA      1689 Б  кросс-подпись, НУЖЕН
                                                             старым клиентам
4  USERTrust RSA CA          ← сам себя              1506 Б  корень, ЛИШНИЙ
```

Корень клиенту бесполезен по определению: кто ему доверяет — уже имеет локально,
кто нет — от присланной копии доверять не начнёт. Третий резать нельзя, и это
проверено по издателям: он не самоподписанный, это Root R46, подписанный
USERTrust, то есть законный промежуточный. Снято **1506 байт с каждого
рукопожатия** — и по h3, и по h2, у всех сервисов на vhost, потому что сертификат
общий (при `initcwnd 10` ≈ 14 КБ это ~11% первого окна).

Перед установкой проверялось: обрезанная цепочка валидируется системным
хранилищем (`openssl verify -CApath /etc/ssl/certs -untrusted`), приватный ключ
соответствует листу. После: `Verify return code: 0`, 3 сертификата по проводу,
живой браузер в secure context без ошибок.

⚠️ **Следующее ручное продление сертификата вернёт корень обратно** — цепочку от
Sectigo надо будет снова обрезать. Автопродления у этого домена нет (`certbot`
обслуживает только `chat.frogsrop.dev`), сертификат до 2027-02-15. Бэкап
оригинала — `/root/watch-nginx-backup/fullchain.crt.20260803-214325`.

**`quic_gso on` отвергнут, и после Фазы 1 аргументов против стало больше.** Он
экономил бы syscalls (~306 `sendmsg` на 413 КБ, h3 стоит 2.3–3.2× CPU на МБ), но
на пяти зрителях это шум на 4 vCPU. А окно теперь в 4 раза больше при полном
отсутствии пейсинга, то есть залп в NIC вырос вчетверо — и работа GSO как раз в
том, чтобы отдать эти пакеты подряд. На пути, чьё узкое место — канал зрителя,
это наводит потери, на которые congestion control 1.26.3 даже не отреагирует
(пол cwnd делает multiplicative decrease no-op).

**`quic_bpf on` отвергнут решающим экспериментом**, а не пропущен: в изолированном
nginx на 8443 четыре воркера дали 16 отказов из 75, один воркер — 15 из 75.
Эффекта воркеров нет, значит BPF-маршрутизации нечего исправлять.
